...
class EmployeeSensitiveDataIntegrationTest {
    ...
    void compensationRequiresExactPermissionAndRejectsOverlappingEffectiveDates() throws Exception {
        long employeeId = createEmployee(adminToken, "Compensation target", branchA.getId()).path("id").asLong();
        ...
        mockMvc.perform(get("/api/employees/{id}/compensation/history", employeeId).header("Authorization", bearer(adminToken)))
    ...
    void identityIsEncryptedMaskedByDefaultAndFullRevealIsSeparatelyAuthorized() throws Exception {
        long employeeA = createEmployee(adminToken, "Identity A", branchA.getId()).path("id").asLong();
        long employeeB = createEmployee(adminToken, "Identity B", branchA.getId()).path("id").asLong();
        ...
        mockMvc.perform(put("/api/employees/{id}/identity", employeeA)
                .header("Authorization", bearer(adminToken)).contentType(MediaType.APPLICATION_JSON).content(request))
        ...
        mockMvc.perform(get("/api/employees/{id}/identity", employeeA).header("Authorization", bearer(adminToken)))
            .andExpect(status().isOk()).andExpect(jsonPath("$.number").value("********1234")).andExpect(jsonPath("$.masked").value(true));
        mockMvc.perform(get("/api/employees/{id}/identity", employeeA).param("reveal", "true").header("Authorization", bearer(adminToken)))
    ...
    void privateDocumentsValidateMagicBytesAndSeparateMetadataFromContentPermission() throws Exception {
        long employeeA = createEmployee(adminToken, "Document A", branchA.getId()).path("id").asLong();
        long employeeB = createEmployee(adminToken, "Document B", branchB.getId()).path("id").asLong();
        ...
        mockMvc.perform(get("/api/employees/{id}/documents/{documentId}/content", employeeA, uploaded.path("id").asLong())
                .param("download", "true").header("Authorization", bearer(adminToken)))
