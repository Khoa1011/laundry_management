...
@Component
public class ActiveEmployeesInBranchResolver implements NotificationAudienceResolver {
    private final NotificationRecipientLookupRepository lookupRepository;

    public ActiveEmployeesInBranchResolver(NotificationRecipientLookupRepository lookupRepository) {
        this.lookupRepository = lookupRepository;
    }

    @Override
    public NotificationAudienceType supports() {
        return NotificationAudienceType.ALL_ACTIVE_EMPLOYEES_IN_BRANCH;
    }

    @Override
    public Set<Long> resolveCandidates(CreateNotificationCommand command) {
        return new LinkedHashSet<>(lookupRepository.findActiveEmployeeUserIdsInBranch(command.branchId()));
    }
}
