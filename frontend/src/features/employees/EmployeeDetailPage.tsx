...
function BranchDialog({ open, onClose, employee }: { open: boolean; onClose: () => void; employee: EmployeeDetail }) {
  ...
  return <OverlayDialog open={open} onClose={onClose} title={t('employee:branchDialogTitle')} footer={<><button className="button button--secondary" onClick={onClose}>{t('cancel')}</button><button className="button button--primary" onClick={() => void submit()} disabled={mutation.isPending || !branchId}>{mutation.isPending ? t('saving') : t('employee:addBranch')}</button></>}><div className="form-stack"><label className="field"><span>{t('employee:branches')}</span><select value={branchId} onChange={(event) => setBranchId(Number(event.target.value))}><option value={0}>{t('employee:allBranches')}</option>{available.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select></label><label className="switch-row"><input type="checkbox" checked={primary} onChange={(event) => setPrimary(event.target.checked)} /><span className="switch" aria-hidden="true" /><span>{t('employee:primaryBranch')}</span></label></div></OverlayDialog>
}
