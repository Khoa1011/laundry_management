...
export function CustomerDetailPage() {
  ...
  return <div className="page-container customer-detail-page">
    ...
    <div className="detail-grid">
      ...
      <section className="content-card detail-addresses"><div className="section-header"><div><h2>{t('addresses:title')}</h2><p>{customer.addresses.length ? t('addresses:count', { count: customer.addresses.length }) : t('addresses:emptyBody')}</p></div>{canAddress && <button className="button button--create" onClick={() => setAddressDialog({ open: true })}><Plus size={18} aria-hidden="true" />{t('addresses:add')}</button>}</div>
        {customer.addresses.length === 0 ? <StatePanel compact icon={<MapPin />} title={t('addresses:emptyTitle')} body={t('addresses:emptyBody')} action={canAddress ? <button className="button button--create" onClick={() => setAddressDialog({ open: true })}><Plus size={18} aria-hidden="true" />{t('addresses:add')}</button> : undefined} /> : <div className="address-list">{customer.addresses.map((address) => <AddressCard key={address.id} address={address} canManage={canAddress} onEdit={() => setAddressDialog({ open: true, address })} onDefault={() => void setDefault(address)} onStatus={() => { setAddressStatusTarget(address); setReplacementId(undefined) }} t={t} />)}</div>}
      </section>
