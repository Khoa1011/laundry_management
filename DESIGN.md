...
- All shared text-button variants use a left-to-right semantic fill on hover, keyboard focus, and press. Primary/create/edit actions fill with the primary blue family, destructive actions fill with the danger red family, success actions fill with green, and neutral actions use a restrained surface tint. Disabled and loading states do not animate, and reduced motion changes state instantly.
...
- Table action menus use the shared portaled fan-out interaction. A compact sky-blue Phosphor trigger expands semantic filled action icons away from the nearest viewport edge over 240ms; the icons begin overlapped with distinct rotations and settle upright. View actions use operational cyan, edit actions use primary sky blue, destructive actions use danger red, success/reactivation actions use green, and generic actions use neutral slate. Fine-pointer controls may use a compact 36px visual size; coarse-pointer and touch controls retain a minimum 44px target. The hover region includes the trigger-to-action corridor so the fan never closes while the pointer remains on the interaction. Only one table action fan may be open at a time; opening another row closes the previous fan immediately. Click and hover must play the same entrance motion; touch, keyboard, Escape, outside press, and reduced motion must all remain usable.
